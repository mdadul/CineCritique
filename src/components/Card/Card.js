  ...
  return (
    ...
    <div className="flex flex-col md:flex-row">
      <div className="p-5 md:w-4/6 md:p-8">
        <span className="rounded-md bg-orange-700 px-2 py-1 text-xs uppercase text-white">{data.Actors}</span>
        <p className="mt-2 text-xl font-black md:mt-6 md:text-4xl">{data.Title}</p>
        <p className="mt-3 text-gray-600">{data.Year}</p>
        <p className="mt-3 text-white bg-gray-900 rounded-xl w-16 px-2 py-1">{data.imdbRating}/10</p>
        <p className="mt-3 text-gray-700">{data.Plot}</p>
        <p className="mt-3 text-gray-900">{data.Genre}</p>
